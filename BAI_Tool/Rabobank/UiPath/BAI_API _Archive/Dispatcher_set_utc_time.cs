/* UiPath Invoke Code Script - Convert Closing Date to UTC Time Range (DST-Aware)
   Input Arguments: ClosingDateText (String, In)
   Output Arguments: dateFromTime (String, Out), dateToTime (String, Out)
   References: System

   Inputs:
     ClosingDateText As String - Date in format "yyyy-MM-dd", "dd-MM-yyyy", or "yyyy/MM/dd"
   Outputs:
     dateFromTime As String - Start of day in UTC (ISO format with Z)
     dateToTime As String - End of day in UTC (ISO format with Z)
*/
...
try
{
    ...
    // Create local time range for the full day (00:00:00 to 23:59:59.999999)
    DateTime localStart = new DateTime(parsed.Year, parsed.Month, parsed.Day, 0, 0, 0, DateTimeKind.Unspecified);
    DateTime localEnd = localStart.AddDays(1).AddTicks(-1);
    ...
    // Format as ISO 8601 strings with Z suffix for API consumption (microsecond precision)
    dateFromTime = utcStart.ToString("yyyy-MM-dd' T'HH:mm:ss.ffffff'Z'", System.Globalization.CultureInfo.InvariantCulture);
    dateToTime = utcEnd.ToString("yyyy-MM-dd' T'HH:mm:ss.ffffff'Z'", System.Globalization.CultureInfo.InvariantCulture);
    ...
    if (enableDebug)
    {
        ...
        System.Console.WriteLine($"[DEBUG] Converted to UTC: {utcStart:yyyy-MM-dd'T'HH:mm:ss.ffffff} to {utcEnd:yyyy-MM-dd'T'HH:mm:ss.ffffff}");
...
/* Output variables:
   dateFromTime: String with start of day in UTC ISO format with Z (e.g., "2025-10-06T22:00:00.000000Z")
   dateToTime: String with end of day in UTC ISO format with Z (e.g., "2025-10-07T21:59:59.999999Z")
*/