...
namespace RabobankZero
{
    ...
    {
        private static readonly string TestDataPath = Path.Combine(Directory.GetCurrentDirectory(), "UiPath", "Testdata", "Conversie standen");
        private static readonly string OutputPath = Path.Combine(Directory.GetCurrentDirectory(), "UiPath", "Testdata");
        ...
        private static void ConvertTransactionData()
        {
            ...
            {
                Console.WriteLine($"Test data path niet gevonden: {TestDataPath}");
                return;
            }
            ...
            foreach (var csvFile in csvFiles)
            {
                ...
                if (parts.Length < 4)
                {
                    Console.WriteLine($"  Ongeldige bestandsnaam format: {fileName}");
                    continue;
                }
                ...
                {
                    Console.WriteLine("  Geen transacties gevonden in CSV");
                    continue;
                }
            ...
            try
            {
                ...
                for (int i = 1; i < lines.Length; i++)
                {
                    ...
                    try
                    {
                        ...
                        if (fields.Length < 26) 
                        {
                            Console.WriteLine($"    Regel {i+1}: onvoldoende velden ({fields.Length}/26)");
                            continue;
                        }
        ...
        private static void ConvertBalanceData()
        {
            ...
            Console.WriteLine("Note: PDF parsing niet geïmplementeerd in deze versie.");
            ...
            foreach (var file in transactionFiles)
            {
                try
                {
                    ...
                    {
                        // Get the last balance from CSV data (more accurate than calculation)
            ...
            foreach (var accountBalances in balancesByAccount)
            {
                ...
                foreach (var (date, balance) in balances)
                {
                    var apiBalance = new ApiBalance
                    {
                        ...
                        currency = "EUR",
                        date = date,
                        openingBalance = balance - (decimal)(new Random().NextDouble() * 100000), // Simulate opening balance
        ...
        {
            // Read from CSV if available, or use dummy calculation
            // For more accurate results, we should parse the actual balance from CSV
