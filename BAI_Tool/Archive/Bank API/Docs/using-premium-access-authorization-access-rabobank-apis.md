...
PSD2 - Consent is valid for 90 days. (After October 5th, it will be valid for 180 days.)
...
You should not embed Rabobank's authorization page in your application, doing this will result in a ban.
...
scope	Yes	scope="bai.accountinformation.read”	
...
For PSD2 - Their consent is valid for 90 days.
...
Access Token
After your application server receives an authorization code, it can exchange it for an access token. This request is made by your application server directly to the Rabobank server.

The access token is valid for 60 minutes for PSD2 and 24 hrs for Premium. Your application must include the access token in any API request that requires authorization from the user.

Access tokens should be stored securely and should only be used on the server side. If your application doesn't require constant access, we recommend to not store the access token.
...
Refresh token
If the access token expires you can obtain a new one by using the refresh token. Your application can use the refresh token to obtain a new access token if:

The user's consent has not been revoked, and
The refresh token has not expired.
rabo_oauth_refresh_flow

The refresh token is valid for 30 days but can only be used one time. It should be stored securely and should only be used on the server side.

When your application obtains a new access token, it also receives a new refresh token.

The expiry time for access tokens is known so your application should be able to predict when it needs to refresh a token.

You should receive information about the access token and refresh token expiry in the Token request response. It is recommended to use the expiration date as specified in the Token response in your application settings.
...
Consent	As per the requested scope
...
The expiration time(s) is not fixed, we do not recommend regarding these values as a fixed value in your settings.

Most Rabobank APIs have a rate limit, making multiple requests at once results in a failure. You should make the requests in multiple batches at a regular time interval.

We recommend your application refreshes the token before it expires, in order to prevent your application from making unsuccessful requests.
...
Receive Tokens
The response contains an access token, a refresh token, their expiration times in seconds, and the timestamp of when the consent was given.
